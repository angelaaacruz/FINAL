...
const ItemList = ({ onEdit, onDelete }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '40px', width: '80%', maxWidth: '610px', backgroundColor: '#DCF2F1', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', marginTop: '40px' }}>
                <h1>RECORDS</h1>
                {error && <p>{error}</p>}
                <ul style={{ listStyleType: 'none', padding: 0 }}>
                    {items.map(item => (
                        <li key={item.id} style={{ marginBottom: '40px', padding: '50px', borderBottom: '2px solid #ccc' }}>
                            ...
                            <button onClick={() => onEdit(item)} style={{ marginRight: '15px' }}>Edit</button>
